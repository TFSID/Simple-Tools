#!/usr/bin/env python3
"""
Task Synthesis System - Python Version
=====================================

Workflow: Create todo list → Input to task → Execute code → Review → Repeat

A comprehensive system for breaking down complex goals into executable tasks,
with AI-powered code generation, analysis, and iterative improvement.
"""
...
class TaskSynthesisManager:
    """
    Main class for managing task synthesis workflows
    """
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    json={
                        'prompt': prompt[:50000],  # Limit prompt size
                    ...
                    logger.info(f"✓ Success ({tokens} tokens)")
    ...
    async def create_task_list(
        ...
        """
        Create a new task list from a goal
        """
...
Priority levels: high, medium, low"""
        ...
        system_instruction = "You are an expert project manager and developer. Return ONLY valid JSON without markdown formatting."

        try:
            result = await self.call_gemini(prompt, system_instruction)
            logger.info(f"Raw API response preview: {result[:200]}...")  # Debug info
            
            # Robust JSON extraction - handle various formats
            json_data = self._extract_json(result)
            logger.info(f"Extracted JSON preview: {json_data[:200]}...")  # Debug info
            ...
            logger.error(f"Raw response: {result[:500] if 'result' in locals() else 'None'}")
            logger.error(f"Extracted JSON: {json_data[:200] if 'json_data' in locals() else 'None'}")
            logger.error(f"JSON Error details: {str(e)}")
            raise ValueError(f"Failed to parse AI response as JSON. Raw response: {result[:200] if 'result' in locals() else 'None'}...")
        except Exception as error:
            logger.error(f"Failed to create task list: {error}")
            logger.error(f"Response was: {result[:200] if 'result' in locals() else 'None'}")
            raise
    
    async def process_next_task(self) -> Optional[TaskItem]:
        """
        Process the next available task in the current task list
        """
...
Keep it concise and actionable."""
    ...
    async def run_full_workflow(
        ...
        logger.info(f"\n🎉 Workflow completed! {completed_tasks} tasks processed.")
        return task_list
    ...
    async def load_task_list(self, task_list_id: str) -> Optional[TaskList]:
        """Load task list from file"""
        file_path = Config.TASKS_DIR / f"{task_list_id}.json"
        ...
        try:
            ...
            task_list = TaskList(**task_list_dict)
            self.task_lists[task_list.id] = task_list
            self.current_task_list = task_list
            
            logger.info(f"📁 Task list loaded: {task_list.title}")
            return task_list
            
        except Exception as error:
            logger.error(f"Failed to load task list: {error}")
            return None

# HTML to Next.js Integration
async def enhanced_html_to_nextjs_conversion(html_file_path: str) -> None:
    """
    Enhanced HTML to Next.js conversion using task synthesis
    """
...
Commands:
  1. Interactive goal setting:
     python task_synthesis_system.py

  2. Convert HTML to Next.js:
     python task_synthesis_system.py convert <html-file-path>

  3. Run custom workflow:
     python task_synthesis_system.py workflow "<goal description>"

  4. Show task list progress:
     python task_synthesis_system.py status

  5. Load existing task list:
     python task_synthesis_system.py load <task-list-id>

Examples:
  python task_synthesis_system.py convert ./input/index.html
  python task_synthesis_system.py workflow "Build a todo app with React and TypeScript"
...
- Persistent task list storage
    ...
    try:
        if command == "convert":
            ...
            print("🎯 Interactive Mode - Setting up synthesis workflow...")
