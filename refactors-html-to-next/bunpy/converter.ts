...
async function callGemini(
  ...
  try {
    ...
    cleanedOutput = cleanedOutput
      // Remove ```typescript or ```tsx or ```ts code blocks
...
Format:
{
  "components": [
    ...
    const systemInstruction = 'You are an expert in React and Next.js component architecture. Return ONLY valid JSON without any markdown formatting or code blocks.';
...
CRITICAL: Return ONLY the raw TypeScript code. 
- NO markdown code blocks (no \`\`\`typescript or \`\`\`)
- NO explanations or comments outside the code
- NO preamble text
- Start directly with the code (either 'use client' or 'export default')`;

    const systemInstruction = 'You are an expert Next.js developer. Return ONLY raw TypeScript code without any markdown formatting, code blocks, or explanations. The response must be valid .tsx file content that can be directly saved.';
    ...
    cleaned = cleaned.replace(/^```(?:typescript|tsx|ts|javascript|jsx|js)?\s*\n?/gm, '');
...
async function convertHTMLToNextJS(htmlFilePath: string) {
  ...
  try {
    ...
    try {
      const pagePrompt = `Create a Next.js page.tsx that imports and uses these components: ${convertedComponents.join(', ')}.

CRITICAL: Return ONLY the raw TypeScript code.
- NO markdown code blocks (no \`\`\`typescript or \`\`\`)
- NO explanations
- Start directly with imports or 'use client'`;

      const pageContent = await callGemini(
        pagePrompt,
        'Return ONLY raw TypeScript code without markdown formatting. The response must be valid .tsx file content.'
      );
      
      // Clean the page content
      let cleanedPage = pageContent.trim();
      cleanedPage = cleanedPage.replace(/^```(?:typescript|tsx|ts)?\s*\n?/gm, '');
      cleanedPage = cleanedPage.replace(/\n?```\s*$/gm, '');
      cleanedPage = cleanedPage.replace(/^```|```$/g, '');
      
      await writeFile(join(outputDir, 'page.tsx'), cleanedPage);
...
- Detailed error logging
  ...
  try {
    ...
    const testResponse = await callGemini('test', 'Respond with just "ok"');
    console.log(`✓ API connection verified (response: ${testResponse.substring(0, 20)}...)\n`);
